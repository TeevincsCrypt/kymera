...
export function TestnetWalletGuard() {
  const { address, chainId, isConnected, status: accountStatus, connector: activeConnector } = useAccount()
  ...
  return <section className="border border-stone-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-5 flex flex-wrap items-center gap-3">{!connected ? <button type="button" onClick={() => setChooserOpen(true)} disabled={isPending} className="bg-stone-950 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{isPending ? 'CONNECTING...' : 'Connect wallet'}</button> : <><span className="border border-stone-200 px-3 py-2 font-mono text-xs text-stone-700">{address.slice(0, 6)}…{address.slice(-4)}</span>{chainId !== kymeraChain.id && <button type="button" onClick={() => switchChain({ chainId: kymeraChain.id })} disabled={switching} className="border border-amber-300 px-4 py-2 text-sm font-medium text-amber-800">{switching ? 'Switching...' : 'Switch to testnet'}</button>}<button type="button" onClick={prepare} disabled={!status.correctNetwork || state === 'CONFIRMING' || state === 'SUBMITTED'} className="bg-stone-950 px-4 py-2 text-sm font-medium text-white disabled:opacity-40">Run Guard & preview</button><button type="button" onClick={() => disconnect()} className="text-sm text-stone-500 underline">Disconnect wallet</button></>}</div>
    {chooserOpen && <div className="mt-4 border border-stone-200 bg-stone-50 p-4"><div className="flex items-center justify-between"><p className="font-mono text-xs uppercase tracking-wider text-stone-700">Connect wallet</p><button type="button" onClick={() => setChooserOpen(false)} className="text-xs text-stone-500 underline">Close</button></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><button type="button" disabled={!injectedConnector || isPending} onClick={() => injectedConnector && connect({ connector: injectedConnector })} className="border border-stone-300 bg-white px-3 py-3 text-left text-sm text-stone-800 disabled:opacity-50"><strong>Injected wallet</strong><span className="mt-1 block text-xs text-stone-500">MetaMask, Rabby, Binance, Trust, Coinbase, and compatible wallets</span></button><button type="button" disabled={!walletConnectConnector || isPending} onClick={() => walletConnectConnector && connect({ connector: walletConnectConnector })} className="border border-stone-300 bg-white px-3 py-3 text-left text-sm text-stone-800 disabled:opacity-50"><strong>WalletConnect</strong><span className="mt-1 block text-xs text-stone-500">{walletConnectConnector ? 'Mobile or desktop wallet' : 'Unavailable — configuration required'}</span></button></div></div>}
    ...
    {process.env.NODE_ENV === 'development' && <div className="mt-4 border-t border-dashed border-stone-200 pt-3 font-mono text-[10px] uppercase tracking-wider text-stone-500"><p>Wallet status: {accountStatus}</p><p>Connector: {activeConnector?.name ?? 'none'}</p><p>Account: {address ?? 'none'}</p><p>Chain ID: {chainId ?? 'none'}</p><p>Available connectors: {connectors.map((candidate) => candidate.name).join(', ') || 'none'}</p><p>Wallet provider: {activeConnector?.name ?? 'none'}</p><p>Connection error: {connectionError?.message ?? 'none'}</p></div>}{state !== 'IDLE' && <p className="mt-4 text-sm text-stone-600">State: <strong>{state}</strong></p>}{error && <p className="mt-2 text-sm text-red-700">{error}</p>}{hash && <p className="mt-2 text-sm text-emerald-700">Transaction {state === 'CONFIRMED' ? 'confirmed' : 'submitted'}: <a className="underline" href={`https://testnet.bscscan.com/tx/${hash}`} target="_blank" rel="noreferrer">{hash}</a></p>}
  </section>
}
