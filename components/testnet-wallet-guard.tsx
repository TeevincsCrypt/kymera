...
export function TestnetWalletGuard() {
  ...
  return <section className="border border-stone-200 bg-white p-5 shadow-sm">
    ...
    <div className="mt-5 flex flex-wrap items-center gap-3">{!connected ? <button type="button" onClick={() => setChooserOpen(true)} disabled={isPending} className="bg-stone-950 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{isPending ? 'CONNECTING...' : 'Connect wallet'}</button> : <><span className="border border-stone-200 px-3 py-2 font-mono text-xs text-stone-700">{address.slice(0, 6)}…{address.slice(-4)}</span>{chainId !== kymeraChain.id && <button type="button" onClick={() => switchChain({ chainId: kymeraChain.id })} disabled={switching} className="border border-amber-300 px-4 py-2 text-sm font-medium text-amber-800">{switching ? 'Switching...' : 'Switch to testnet'}</button>}<button type="button" onClick={prepare} disabled={!status.correctNetwork || state === 'CONFIRMING' || state === 'SUBMITTED'} className="bg-stone-950 px-4 py-2 text-sm font-medium text-white disabled:opacity-40">Run Guard & preview</button><button type="button" onClick={() => { clearSelectedEip6963Provider(); setSelectedWallet(null); setPreview(null); setHash(''); setState('IDLE'); disconnect() }} className="text-sm text-stone-500 underline">Disconnect wallet</button></>}</div>
    {chooserOpen && <div className="mt-4 border border-stone-200 bg-stone-50 p-4"><div className="flex items-center justify-between"><p className="font-mono text-xs uppercase tracking-wider text-stone-700">Connect wallet</p><button type="button" onClick={() => setChooserOpen(false)} className="text-xs text-stone-500 underline">Close</button></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{wallets.map((wallet) => <button key={wallet.info.uuid} type="button" disabled={isPending} onClick={() => chooseWallet(wallet)} className="flex items-center gap-3 border border-stone-300 bg-white px-3 py-3 text-left text-sm text-stone-800 disabled:opacity-50"><img src={wallet.info.icon} alt="" className="size-7 rounded-md" /><span><strong>{wallet.info.name}</strong><span className="mt-1 block text-xs text-stone-500">Select this wallet</span></span></button>)}{wallets.length === 0 && <p className="border border-dashed border-stone-300 px-3 py-3 text-xs text-stone-500">No EIP-6963 wallets announced yet. Install a compatible wallet or use WalletConnect.</p>}<button type="button" disabled={!walletConnectConnector || isPending} onClick={() => walletConnectConnector && connect({ connector: walletConnectConnector })} className="border border-stone-300 bg-white px-3 py-3 text-left text-sm text-stone-800 disabled:opacity-50"><strong>WalletConnect</strong><span className="mt-1 block text-xs text-stone-500">{walletConnectConnector ? 'Mobile or desktop wallet' : 'Unavailable — configuration required'}</span></button></div></div>}
